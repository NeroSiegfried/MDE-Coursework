...
entryRuleDummy returns [String current=null]:
	{ newCompositeNode(grammarAccess.getDummyRule()); }
	iv_ruleDummy=ruleDummy
	{ $current=$iv_ruleDummy.current.getText(); }
	EOF;

// Rule Dummy
ruleDummy returns [AntlrDatatypeRuleToken current=new AntlrDatatypeRuleToken()]
@init {
	enterRule();
}
@after {
...
@after {
	...
	(
		...
		{
			$current = $this_Checkmate_2.current;
			afterParserOrEnumRuleCall();
		}
	)
;
...
entryRuleSquare returns [String current=null]:
	{ newCompositeNode(grammarAccess.getSquareRule()); }
	iv_ruleSquare=ruleSquare
	{ $current=$iv_ruleSquare.current.getText(); }
	EOF;

// Rule Square
ruleSquare returns [AntlrDatatypeRuleToken current=new AntlrDatatypeRuleToken()]
@init {
	enterRule();
}
@after {
	...
	this_SQUARETERMINAL_0=RULE_SQUARETERMINAL
	{
